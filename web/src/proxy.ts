 ...
 * middleware`, see `web/README.md`'s journal entry) — deliberately narrow.
 *
 ...
 * (`web/src/shared/lib/auth/guard.ts`), declared right next to each
...
async function enforceRateLimit(request: NextRequest, pathname: string): Promise<NextResponse | null> {
    ...
    if (pathname === "/api/auth/refresh") {
        ...
        // well as per-IP, inside the route (web/src/app/api/auth/login/
