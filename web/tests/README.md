...
> `frontend/tests/` (the legacy Vite SPA) once `web/` (Next.js + Postgres) became the app that
...
This repo has automated screenshot (visual-regression) and accessibility testing for `web/`, the
Next.js app that has replaced the legacy Vite SPA (`frontend/`) in production. It exists to catch
...
in this repo (`web/tests/visual-snapshots/`) — that's the "references" folder. Identical tooling
choice to `frontend/tests/` — nothing here needed to change to fit Next.js.
...
**What actually changed porting this from `frontend/tests/` to `web/tests/`:** the tests
...
import (`@/data/work`, `@/data/journal`); `web/`'s pages live in Postgres, fetched through
...
- Node.js 20+ (`backend-web-checks.yml`, the CI workflow covering `web/`, pins **Node 22** because
  `@stryker-mutator/vitest-runner` needs it — this suite doesn't have its own separate Node
  requirement, it just rides along with whatever `web/`'s other CI already uses). Locally this was
...
```bash
# from the repo root — web/ and backend/ are npm workspace members, installed together
npm install
cd web
...
```bash
# web/.env.test (copy from .env.test.example — gitignored, never commit your own)
...
`web/`'s content lives in Postgres. Building the equivalent list means calling `getAllWork()` /
...
3. **Reproduce the CI environment locally via Docker**, if you specifically want to iterate on a
   ...
   (Match the image tag's Playwright version to `web/package.json`'s `@playwright/test` version
...
unlike `frontend/tests/`, dependencies are installed once at the **repo root** (`npm ci`, no
`working-directory`), since `web`/`backend` are npm workspace members sharing one root
`package-lock.json` — `frontend/` had its own standalone lockfile and could install in place.
...
`web/test-results/summary.json` (the file our custom `summary-reporter.ts` writes).
...
`web/`-specific change once the workflow itself is retargeted.)
...
3. Bump in `web/package.json`, then run `npm install` (or `npm update @playwright/test
...
design tokens/components themselves (ported to `web/src/shared/ui/theme/`), not in this test
...
re-checking once this port's first real `test:a11y` run against `web/`'s landing page completes,
...
`JournalListPage.tsx` (ported to `web/src/views/journal-list/`), not in this test suite.
...
changed, what didn't work and how it was fixed. Entries before the "port to web/tests/" heading
...
### 2026-07-27 — Port to web/tests/ (this port)
...
- Ported `visual.spec.ts`, `a11y.spec.ts`, `utils/theme.ts`, `reporters/summary-reporter.ts`
  ...
  Confirmed `web/`'s theme storage key (`portfolio.theme-preference`,
  `web/src/shared/theme/theme.context.tsx`) matches `frontend/`'s exactly, so `utils/theme.ts`
  needed zero changes.
...
- Added `web/tests/tsconfig.json` as its own small TS project, and excluded `tests/` from
  `web/tsconfig.json`'s `include` set — confirmed (via Next.js's own docs, not assumed) that `next
  ...
  build` step in `web/` — for reasons having nothing to do with the app itself.
...
- Left CI retargeting (`.github/workflows/visual-tests.yml`,
  ...
  rather than describing CI as already wired to `web/tests/`, which it isn't yet.
...
3. **MSW + Next.js's experimental `testProxy`**, intercepting `fetch()` during SSR. Confirmed (by
   ...
   as currently architected**: `web/`'s public Server Components call `@portfolio/backend`
   ...
   this one.
4. **Storybook-level component isolation** (`web/src/views/storybook/` already exists as a
...
### 2026-07-27 — CI retargeted from `frontend/tests/` to `web/tests/`
...
legacy Vite SPA), even though the suite itself had already been ported to `web/tests/` (the entry
above). Sections 7/8 of this document explicitly called out this gap as a caveat rather than
...
`web/tests/`'s data source is a real Postgres database (unlike `frontend/tests/`'s static
...
- Changed dependency install from `working-directory: frontend` + its own `package-lock.json` to a
  **root-level** `npm ci` with `cache-dependency-path: package-lock.json` — `web`/`backend` are npm
  workspace members sharing one root lockfile; `frontend/` was a standalone app with its own.
  **This deviates from the retirement plan's literal text** (which said `cache path →
  web/package-lock.json`), because no such file exists or should exist once `web/` is a workspace
...
- `.github/scripts/format-summary.mjs` hardcoded its OUTPUT path as `frontend/pr-comment.md` in
  ...
  `web/pr-comment.md` to match the workflow's own `Comment on PR` step.
...
**Not yet done.** Since no baselines exist yet in `web/tests/visual-snapshots/` (the port's own
