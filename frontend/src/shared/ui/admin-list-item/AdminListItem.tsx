            ...
            <div className="flex-1 flex flex-col gap-sm min-w-0">
                <div className="flex items-center justify-between gap-sm">
                    ...
                    <Text variant="caption" tone="faint"
                          className="font-mono whitespace-nowrap shrink-0">{ meta }</Text>
                </div>

                <div className="flex items-start justify-between gap-md">
                    <Text variant="body" className="font-medium line-clamp-3 min-w-0">{ title }</Text>
                    <div className="flex items-center gap-xs shrink-0">
                        <RevealAction as="link" href={ editHref } label="Edit"
                                      icon={ <Pencil className="h-md aspect-square w-auto" aria-hidden="true"/> }/>
                        <RevealAction as="button" onClick={ onDelete } loading={ deleting } label="Delete" tone="danger"
                                      icon={ <Trash2 className="h-md aspect-square w-auto" aria-hidden="true"/> }/>
                    </div>
                </div>
...
function RevealAction(props: RevealActionProps) {
    ...
    const className = cn(
        "group relative inline-flex h-2xl aspect-square w-auto shrink-0 items-center justify-center overflow-hidden rounded-md",
        ...
        "hover:w-(--spacing-5xl) focus-visible:w-(--spacing-5xl)",
    ...
    return (
        <button type="button" aria-label={ label } onClick={ props.onClick } disabled={ loading }
                className={ className }>
            { content }
        </button>
    );
