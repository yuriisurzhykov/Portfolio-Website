...
- [Procedural covers for posts and work items](#procedural-covers-for-posts-and-work-items)
...
## Procedural covers for posts and work items

Every Journal post and Work project gets a cover image the moment it is
created — not uploaded by hand, not fetched from a stock-photo API, not
generated by a model, but computed by a pure function of the post itself.
That constraint was the whole point: a personal blog with one administrator
has no budget for "go find a picture" as a recurring chore, yet a wall of
identical placeholder rectangles would be its own kind of failure. The
answer that satisfies both is a deterministic pipeline, laid out below.

![Procedural cover generation pipeline](docs/images/06-cover-generation.svg)

Two decisions carry the rest of the design. First, a cover's **hue** is not
a hash of its category or slug — a pure hash of one string cannot promise
that two different categories end up visually distinguishable, since
distinguishability is a property of the whole set of hues in use, not of
any single string. Instead, `resolveIdentityHue()` hands out hues one at a
time from a van der Corput, base-2 sequence, out of a single ordinal pool
shared by post categories *and* Work projects, so a post and a project can
never accidentally end up the same color — the actual bit-reversal doing
the work, worked through by hand for a real ordinal, looks like this:

![Van der Corput hue-assignment algorithm, worked by hand](docs/images/07-hue-algorithm.svg)

That closed-form bit reversal is what turns "assign hue N" into "always
bisect the largest remaining gap on the circle" without ever having to look
at hues already taken, which is exactly the property a pure hash of one
string cannot offer. Second, everything about the cover's actual geometry —
the mesh gradient's spots, the flow curves, the letterform cut from the
title's first word — is seeded from
`` `${slug}:${styleVersion}:${variant}` `` through a small hand-rolled PRNG
(`cyrb128` → `sfc32`, deliberately not `Math.random`), so the very same post
renders the same cover byte-for-byte on every machine and after every
redeploy — a property worth having for a system with no "regenerate all
covers" button to fix drift after the fact.

The SVG that comes out of that pipeline is rasterized to WebP once, on the
server, rather than shipped as SVG and redrawn in every visitor's browser:
a list of a dozen journal cards is a dozen gradients recomputed on every
scroll if the browser has to do it, and zero if a `sharp`/librsvg render
already did it once at publish time. That same cover is deliberately **not**
reused as the `og:image` a chat app unfurls when a link is shared — a
messenger's preview needs a readable title, not a decorative gradient, so
`og:image` stays its own small text-card renderer (`next/og`), sharing only
the accent hue with the cover it sits next to. The full account of this
design — including the alternatives it replaced, like six composition
archetypes and a golden-angle hue sequence, and why each one lost — lives in
[`backend/src/media/README.md`](backend/src/media/README.md).
...
- [`backend/src/media/README.md`](backend/src/media/README.md) — the procedural cover pipeline: deterministic seeding, hue assignment, and why it's not the OG image.
