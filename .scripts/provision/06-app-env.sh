#!/usr/bin/env bash
# Writes /srv/apps/yuriisoft-web/shared/.env — the one file holding real
# production secrets (DATABASE_URL, JWT_ACCESS_SECRET, JWT_REFRESH_SECRET).
# Owned by nextapp, mode 600 — the deploy account itself (yuriisoft) cannot
# read it back after writing it.
#
...
#   JWT_ACCESS_SECRET / JWT_REFRESH_SECRET (two DIFFERENT values):
...
ENV_FILE="/srv/apps/yuriisoft-web/shared/.env"
APP_USER="nextapp"
...
DATABASE_URL="postgresql://portfolio:${PORTFOLIO_DB_PASSWORD}@127.0.0.1:5432/portfolio"
